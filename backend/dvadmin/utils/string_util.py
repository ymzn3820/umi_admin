...
def convert_to_desired_format(dt_str):
    # 使用字符串创建 datetime 对象
    dt_obj = datetime.strptime(str(dt_str), "%Y-%m-%d %H:%M:%S%z")

    # 使用strftime格式化datetime对象为"2023-09-28 16:00:00"格式
    formatted_dt_without_tz = dt_obj.strftime("%Y-%m-%d %H:%M:%S")

    return formatted_dt_without_tz