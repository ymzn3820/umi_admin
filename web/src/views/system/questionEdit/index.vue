...
export default {
  ...
  methods: {
    renderContent (h, { row }) {
      ...
      if (row.content && row.content.length > maxContentLength) {
        const truncatedContent = row.content.substring(0, maxContentLength) + '...'
        const fullContent = row.content
        return h('div', [
          h('span', truncatedContent),
          h(
            'a',
            {
              on: {
                mouseenter: () => {
                  this.$set(row, 'showFullContent', true)
                },
                mouseleave: () => {
                  this.$set(row, 'showFullContent', false)
                }
              }
            },
            row.showFullContent ? fullContent : ''
          )
        ])
      }
