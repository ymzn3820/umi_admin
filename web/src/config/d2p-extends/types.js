      ...
      if (value != null) {
        if (value.length >= 0) {
          if (value instanceof Array) {
            ...
            row[col.key] = value.replace('https://umi-intelligence.oss-cn-shenzhen.aliyuncs.com/', '')
          } else {
            row[col.key] = value
          }
        } else {
      ...
      if (value != null && value) {
        ...
        row[col.key].map((val, index) => {
          if (val.startsWith('/api')) {
            ...
            row[col.key][index] = 'https://umi-intelligence.oss-cn-shenzhen.aliyuncs.com/' + val
          } else {
          }
        })
      }
